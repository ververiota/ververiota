  ...
  if (senderID == global.data.botID) return;
    ...
    var thu = moment.tz('Asia/Ho_Chi_Minh').format('dddd');
     let name = await Users.getNameUser(senderID);
      if(getMsg.attachment[0] == undefined) return api.sendMessage(`Hôm nay là ${ngay} (${thu})\nVà ${name} đã cố giấu 1 tin nhắn vào khoảng thời gian ${gio} \nVới nội dung là: ${getMsg.msgBody}`,threadID)
      else {
            let num = 0
            let msg = {
              body:`Hôm nay là ${ngay} (${thu})\nVà ${name} đã cố giấu ${getMsg.attachment.length} tệp đính kèm vào lúc ${gio}.${(getMsg.msgBody != "") ? `\n\nNội dung: ${getMsg.msgBody}` : ""}`,
              attachment:[],
        ...
        var path = __dirname + `/resend/${num}.${ext}`;
