...
var API_KEY = "mzk_ZBMGRM2ZMMGDNA71OVP"; //Nhập API_Key của bạn ở đây/*mzkFree_C9B3829923606*/
var timeVD = "0"; // Nhập thời lượng video (là 1 con sô)
    ...
    try {
        ...
        if (args.join(" ").indexOf("https://") == 0) {
            ...
            try {
                ...
                let { data } = await axios.get(`http://manhict.tech/tiktok?link=${linkurl}&apikey=${API_KEY}`);
            ...
            try {
                ...
                results = (await axios.get(`http://manhict.tech/tiktok/search?query=${keywordSearch}&time=${timeVD}&apikey=${API_KEY}`)).data.results;
                for (let key of results) {
                  ...
                  if (num == 10) var num1 = "10"
                    msg += (`${num1}.Thời gian:${time} • ${key.desc}\n\n`);
                }
                var body = `🔎 Có ${link.length} từ khóa trùng khớp với tìm kiếm của bạn:\n\n${msg}Hãy reply (phản hồi) với những từ khóa tìm kiếm trên`;
